...
| `enable_fused_mc2`                  | int  | `0`     | Fused MC2 configuration. Can also be configured via the `VLLM_ASCEND_ENABLE_FUSED_MC2` environment variable during the migration period. On eligible A2 BF16 models, mode `1` reserves the fused input, active-mask, and routed-token domains for the complete `max_num_batched_tokens` scheduler limit at startup; scheduler-legal batches do not switch communication family at runtime. This reservation grows with the scheduler limit, EP size, and MoE top-k. |
