#!/usr/bin/env python3
"""Standalone v4 patch for explicit nano-PEARL verification wiring.

v3 found the correct custom_class call, but the current HUST ModelRunner
already contains a ``request_ids=...`` keyword.  v4 adds only missing
keywords, so it cannot generate a duplicate ``request_ids`` argument.
"""
...
def _next_line_indent(source: str, position: int) -> str:
    ...
    if line_end < 0:
        return ""
    match = re.match(r"[ \t]*", source[line_end + 1:])
...
def transform_runner(source: str) -> str:
    ...
    # request_ids may have been added by the earlier request-ID wiring.
    # Inject only keywords that are absent from this particular call.
    missing = []
    proposer_body = source[proposer_start: proposer_close + 1]
    if not re.search(r"\brequest_ids\s*=", proposer_body):
        missing.append(
            "request_ids=getattr(self.input_batch, \"req_ids\", None),"
        )
    if not re.search(r"\bverify_results\s*=", proposer_body):
        missing.append(
...
def main() -> int:
    ...
    print("change: explicit per-request Target verification for nano-PEARL v4")
    if not changed:
    ...
    backup_dir = (
        ...
        else repo.parent / f"{repo.name}.pearl_stage5_nanoparl_explicit_verify_v4.{stamp}"
    )
    ...
    for relative in changed:
        target = repo / relative
        temporary = target.with_name(target.name + ".pearl_stage5_v4_tmp")
