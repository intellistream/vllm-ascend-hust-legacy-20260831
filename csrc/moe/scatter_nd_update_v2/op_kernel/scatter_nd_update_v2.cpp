...
extern "C" __global__ __aicore__ void scatter_nd_update_v2(GM_ADDR varRef, GM_ADDR indices,
    ...
    // indexType: 1=int32/int32 offsets, 2=int64 cast/int32 offsets,
    //            3=int64/int64 offsets, 4=int32/int64 offsets
    // sortFlag: 0=非排序, 1=排序
    if (TILING_KEY_IS(11)) {
        ...
        ScatterNdUpdateV2::LargeIndexKernel<DTYPE_VAR, int64_t> op(
            indices, updates, output, tilingData, tpipe);
        op.Process();
    } else if (TILING_KEY_IS(40)) {
        ScatterNdUpdateV2::LargeIndexKernel<DTYPE_VAR, int32_t> op(
            indices, updates, output, tilingData, tpipe);
        op.Process();
    }
#endif
