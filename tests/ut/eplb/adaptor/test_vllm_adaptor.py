...
class TestVllmAdaptor(unittest.TestCase):
    ...
    def test_init_fp16_with_parameter_accessor(self, mock_get_config, mock_func):
        ...
        self.assertIs(adaptor.param_dict["0.w13_weight"], self.mock_layer.w13_weight)
        self.assertIs(adaptor.param_dict["0.w2_weight"], self.mock_layer.w2_weight)
        self.assertTrue(torch.equal(adaptor.expert_param_per_layer[0][0][0], self.mock_layer.w13_weight[0]))
        self.assertTrue(torch.equal(adaptor.expert_param_per_layer[0][0][1], self.mock_layer.w2_weight[0]))
        self.mock_layer.get_eplb_parameter.assert_has_calls([call("w13_weight"), call("w2_weight")])
