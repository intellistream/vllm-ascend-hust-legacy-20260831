...
SCRIPT_PATH = Path(__file__).resolve().parents[2] / ".github/workflows/scripts/resolve_perfgate_spec_file.py"
...
""".lstrip(),
    ...
    for spec_file in (
        ...
        "docs/official-baselines/custom-same-spec-910b2.json",
    ):
...
def test_resolve_values_preserves_explicit_same_spec_override(
    tmp_path: Path,
) -> None:
    resolver = load_resolver()
    benchmark_repo = write_fake_benchmark_repo(tmp_path)
    same_spec_file = "docs/official-baselines/custom-same-spec-910b2.json"

    values = resolver.resolve_values(
        explicit_same_spec_file=same_spec_file,
        explicit_perfgate_spec_file="",
    ...
    assert values["SAME_SPEC_SPEC_FILE"] == str((benchmark_repo / same_spec_file).resolve())
    assert "PERFGATE_SPEC_FILE" not in values
    assert values["HARDWARE_CHIP_MODEL"] == "910B2"
    assert values["SOC_VERSION"] == "ascend910b2"


def test_resolve_values_prefers_explicit_perfgate_override(
    tmp_path: Path,
) -> None:
    resolver = load_resolver()
    benchmark_repo = write_fake_benchmark_repo(tmp_path)
    perfgate_spec_file = "docs/official-baselines/perfgate-random-910b2.json"

    values = resolver.resolve_values(
        explicit_same_spec_file="",
        explicit_perfgate_spec_file=perfgate_spec_file,
        scenario="sharegpt-online",
        explicit_chip_model="",
        npu_smi_bin="",
        benchmark_repo=str(benchmark_repo),
    ...
    assert values["PERFGATE_SPEC_FILE"] == perfgate_spec_file
    assert values["SAME_SPEC_SPEC_FILE"] == str((benchmark_repo / perfgate_spec_file).resolve())
    assert values["HARDWARE_CHIP_MODEL"] == "910B2"


def test_resolve_values_uses_shared_registry_for_scenario_and_chip(
    tmp_path: Path,
) -> None:
    resolver = load_resolver()
    benchmark_repo = write_fake_benchmark_repo(tmp_path)
    ...
    assert values["PERFGATE_SPEC_FILE"] == "docs/official-baselines/perfgate-sharegpt-910b2.json"
    assert values["SAME_SPEC_SPEC_FILE"] == str(
        (benchmark_repo / "docs/official-baselines/perfgate-sharegpt-910b2.json").resolve()
    )
    assert values["HARDWARE_CHIP_MODEL"] == "910B2"


def test_resolve_values_fails_closed_for_unknown_chip_model(
    tmp_path: Path,
) -> None:
    resolver = load_resolver()

    with pytest.raises(ValueError, match="No perfgate spec registered"):
        resolver.resolve_values(
            explicit_same_spec_file="",
            explicit_perfgate_spec_file="",
            scenario="random-online",
            explicit_chip_model="910B3",
            npu_smi_bin="",
            benchmark_repo=str(write_fake_benchmark_repo(tmp_path)),
        )


def test_main_writes_github_env(
    tmp_path: Path,
    ...
    assert "PERFGATE_SPEC_FILE=docs/official-baselines/perfgate-random-910b2.json" in env_text
    assert (
        "SAME_SPEC_SPEC_FILE="
        f"{(benchmark_repo / 'docs/official-baselines/perfgate-random-910b2.json').resolve()}" in env_text
    )
