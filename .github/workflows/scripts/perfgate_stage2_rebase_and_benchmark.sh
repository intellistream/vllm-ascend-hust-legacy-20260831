...
STAGE2_RESULTS_ROOT=${BENCHMARK_RESULTS_ROOT:-${GITHUB_WORKSPACE:-$PWD}/.benchmarks/ci}
STAGE2_RESULT_ROOT=${PERFGATE_STAGE2_RESULT_ROOT:-$STAGE2_RESULTS_ROOT/$STAGE2_RUN_ID}
...
run_stage2_benchmark() {
  ...
  while [[ "$attempt" -le "$max_attempts" ]]; do
    ...
    RUN_ID="$STAGE2_RUN_ID" \
      ...
      SUBMISSIONS_ROOT="$STAGE2_RESULT_ROOT/submissions" \
      SUBMISSION_DIR="$STAGE2_RESULT_ROOT/submissions/$STAGE2_RUN_ID" \
      PUBLISH_TO_HF=0 \
      PUBLISH_TO_BENCHMARK_REPO=0 \
      SYNC_GITHUB_SNAPSHOTS=0 \
    ...
    if [[ "$rc" -eq 86 && "$attempt" -lt "$max_attempts" ]]; then
      ...
      if [[ -f .github/workflows/scripts/cleanup_ascend_ci_processes.sh ]]; then
