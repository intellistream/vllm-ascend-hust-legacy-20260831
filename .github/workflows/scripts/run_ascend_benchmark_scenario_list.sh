...
if [[ "$overall_exit_code" -ne 0 ]]; then
  while IFS=$'\t' read -r scenario scenario_run_id scenario_result_root raw_result submission_dir exit_code; do
    ...
    print_file_tail "runtime readiness log" "${scenario_result_root}/runtime-ready.log" 80
    print_file_tail "benchmark diagnostics" "${scenario_result_root}/benchmark_diagnostics.md" 120
    echo "::endgroup::"
  done < "$summary_file"
fi
