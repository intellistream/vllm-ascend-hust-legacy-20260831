...
BASELINE_FILE=${PERFGATE_BASELINE_SOURCE_FILE:-$RESULT_ROOT/submissions/$RUN_ID/plugin-run_leaderboard.json}
...
if ! [[ "$TARGET_SHA" =~ ^[0-9a-f]{40}$ ]]; then
  echo "PERFGATE_TARGET_SHA must be a full lowercase 40-character Git SHA" >&2
  exit 2
fi
...
if [[ ! "$TARGET_REPOSITORY" =~ ^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$ ]]; then
  echo "Invalid target repository for HTTPS main verification: $TARGET_REPOSITORY" >&2
  exit 2
fi
