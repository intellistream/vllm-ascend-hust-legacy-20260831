...
still has to be stored somewhere. Existing MoE offloading systems typically
treat expert weights as dynamically cached device objects and reduce transfer
costs through cache replacement, host-to-device prefetching, and stream-level
copy/compute overlap. This formulation is insufficient for Ascend NPUs. Under
limited HBM, dynamically loading experts into arbitrary device buffers conflicts
with the stable weight addresses, fixed execution windows, graph replay, and
explicit movement scheduling that Ascend execution relies on. As a result,
offloaded MoE inference either exposes host-to-HBM expert loading on the
critical path or gives up the static execution regularity needed for efficient
NPU execution. This document describes the design of SEW-Offload, a static
expert-window scheduling system for HBM-limited MoE inference on Ascend NPUs.
SEW-Offload does not retrain the
...
expert is loaded into arbitrary weight tensors at runtime, the system may save
HBM capacity but lose the regularity needed for efficient NPU execution.
...
Existing MoE offloading systems treat expert weights as dynamically cached
device objects and optimize residency mainly through cache replacement and
prefetch prediction. This abstraction is incomplete for Ascend NPUs. Once HBM
is insufficient to keep all experts resident, dynamic expert loading conflicts
with the NPU's preference for stable weight addresses, fixed execution windows,
graph/static-kernel reuse, and explicitly scheduled data movement. As a result,
offloaded MoE inference either exposes host-to-HBM expert loading on the
critical path or gives up the static execution regularity that makes Ascend
efficient.

The central problem is the mismatch between dynamic expert working sets and
static NPU execution windows under limited HBM. GPU-style MoE offloading
optimizes which experts are cached, but Ascend MoE offloading also depends on
where experts reside, when they are loaded, and whether their loading can be
hidden behind static, grouped NPU execution.
