...
class AscendSpecDecodeBaseProposer(SpecDecodeBaseProposer):
    ...
    def __init__(self, vllm_config: VllmConfig, device: torch.device, pass_hidden_states_to_model: bool, runner=None):
        ...
        # PEARL DEBUG: EAGLE3 final AC stats, read by test_eagle3_offline.py.
        self._eagle_ac_stats_path = os.environ.get(
            "EAGLE_AC_STATS_PATH",
            "/tmp/eagle_ac_stats.json",
        )
        self._eagle_ac_cpu_total_draft_tokens = 0
        self._eagle_ac_cpu_total_accept_tokens = 0

        # PEARL DEBUG: final EAGLE3 acceptance statistics.
        # Keep counters on device to avoid per-step CPU sync.
        self._eagle_ac_total_draft_tokens = torch.zeros(
            (), dtype=torch.int64, device=device
        )
        self._eagle_ac_total_accept_tokens = torch.zeros(
            (), dtype=torch.int64, device=device
        )
        self._eagle_ac_printed = False

    def _accumulate_eagle_ac(
        self,
        ...
        # valid_sampled_tokens_count includes the bonus token generated by target.
        # AC only counts accepted draft tokens, so subtract 1.
        accepted_draft_tokens_gpu = torch.minimum(
            torch.clamp(
                valid_sampled_tokens_count.to(torch.int64) - 1,
                min=0,
            ),
            num_draft_tokens_gpu.to(torch.int64),
        )

        draft_delta = int(num_draft_tokens_gpu.to(torch.int64).sum().item())
        accept_delta = int(accepted_draft_tokens_gpu.sum().item())

        self._eagle_ac_cpu_total_draft_tokens += draft_delta
        self._eagle_ac_cpu_total_accept_tokens += accept_delta

        path = getattr(self, "_eagle_ac_stats_path", None)
        if path:
            tmp_path = f"{path}.{os.getpid()}.tmp"
            with open(tmp_path, "w", encoding="utf-8") as f:
                json.dump(
                    {
                        "draft_tokens": self._eagle_ac_cpu_total_draft_tokens,
                        "accepted_tokens": self._eagle_ac_cpu_total_accept_tokens,
                    },
                    f,
                )
            os.replace(tmp_path, path)

    def _print_eagle_ac_summary(self) -> None:
        if getattr(self, "_eagle_ac_printed", False):
            return

        self._eagle_ac_printed = True

        draft_counter = getattr(self, "_eagle_ac_total_draft_tokens", None)
        accept_counter = getattr(self, "_eagle_ac_total_accept_tokens", None)

        if draft_counter is None or accept_counter is None:
            return

        draft_tokens = int(draft_counter.item())
        accepted_tokens = int(accept_counter.item())

        if draft_tokens <= 0:
            return

        ac_rate = accepted_tokens / max(draft_tokens, 1)
