...
def select_moe_comm_method(num_tokens: int, vllm_config: VllmConfig, is_draft_model=False) -> MoECommType | None:
    ...
    3. On A2 with expert parallel, use the floating-point fused MC2 path when
       ...
       the kernel pipeline and the token count is within the kernel's supported
       range and MC2 capacity. Otherwise, pick MC2 when tokens fit the MC2
    ...
    if not vllm_config.parallel_config.enable_expert_parallel or get_ep_group().world_size == 1:
        ...
        fused_float_enable = (
            ...
            and MIN_A2_FUSED_MC2_TOKENS <= num_tokens <= mc2_tokens_capacity
        )
