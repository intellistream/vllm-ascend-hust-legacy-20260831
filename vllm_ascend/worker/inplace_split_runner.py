...
_INPLACE_LAZY_CAPTURE_LOCK = threading.Lock()
...
class InplaceSplitRunner:
    ...
    def _stabilize_inplace_common_attn_metadata_list(
        ...
        *,
        ...
        mrope_positions_gpu = getattr(getattr(self._runner, "mrope_positions", None), "gpu", None)
        positions_gpu = getattr(getattr(self._runner, "positions", None), "gpu", None)
    ...
    def inplace_split_precheck_reason(
        ...
        *,
        ...
        from vllm_ascend.worker.inplace_split_utils import (
            ...
            inplace_split_runtime_rejection,
        )

        mode = split_batch_config.mode
        runtime_rejection = inplace_split_runtime_rejection(
            enabled=split_batch_config.enabled,
            mode=mode,
            parallel_streams=split_batch_config.enable_parallel_streams,
            full_graph=cudagraph_mode == CUDAGraphMode.FULL,
            uniform_decode=uniform_decode,
            spec_decode=(spec_decode_enabled or split_batch_config.enable_inplace_spec_decode),
            dp_world_size=dp_world_size,
            has_gdn_or_hybrid=has_gdn_or_hybrid,
    ...
    def should_split(
        ...
        uniform_decode = (
            len(num_scheduled_tokens_np) == num_reqs
            and self.uniform_decode_query_len > 0
            and int(np.sum(num_scheduled_tokens_np)) == num_reqs * self.uniform_decode_query_len
            and bool(np.all(num_scheduled_tokens_np == self.uniform_decode_query_len))
        )
        ...
        if split_plan is not None and not inplace_split_first_graph_matches_attention_backend(
            split_plan,
    ...
    def _expand_inplace_inputs_for_graph_slice(
        ...
        input_ids = maybe_expand_tensor_for_graph_slice(input_ids, input_ids_backing, graph_stop, name="input_ids")
        ...
        if positions is not None:
            ...
            positions = maybe_expand_tensor_for_graph_slice(
                positions, positions_backing, graph_stop, name="positions", token_dim=positions_token_dim
            )
    ...
    def _fill_inplace_padding_tail(
        ...
        *,
        ...
        def _fill() -> dict[str, Any]:
            ...
            if positions is not None:
                positions_token_dim = 1 if positions.ndim == 2 else 0
                fill_tensor_token_tail(positions, tail_slice, name="positions", token_dim=positions_token_dim)
    ...
    def _prepare_inplace_split_inputs_for_execution(
        ...
        for idx, split_slice in enumerate(split_batch_slices):
            ...
            split_input_ids, split_positions, split_inputs_embeds = self._expand_inplace_inputs_for_graph_slice(
            ...
            padding_tail_payload = self._fill_inplace_padding_tail(
                split_slice, split_input_ids, split_positions, split_inputs_embeds, intermediate_tensors, stream=stream
            )
            prepared.append(
                {
                    "tokens_slice": ts,
    ...
    def needs_inplace_serial_offset_capture(self, metadata: AscendUbatchMetadata) -> bool:
    ...
    @contextmanager
    def _bind_inplace_parallel_rope_capture_slot(self, context: Any, *, parallel_streams: bool):
        if not parallel_streams or getattr(context, "split_inplace_mode", "") != "inplace_parallel":
            yield
            return
        ...
        if cos is None or sin is None:
            raise RuntimeError(f"Missing rotary cos/sin buffers for inplace parallel capture slot {slot_id}")
    ...
    def _run_inplace_serial_offset_capture(
        ...
        *,
        ...
        capture_stream = self.stream_parallel if parallel_streams else self.stream_main
        ...
        if is_offset:
            ...
            logger.debug(
                "[inplace_serial] lazy capture: forced_attention_backend=%s, "
                "inplace_force_pa_for_offset=%s, start_num_tokens=%d",
                forced_backend,
                split_cfg.inplace_force_pa_for_offset,
        ...
        previous_capturing_enabled = compilation_monitor.cudagraph_capturing_enabled
        compilation_monitor.set_cudagraph_capturing_enabled(True)

        try:
            with (
                self._bind_inplace_parallel_rope_capture_slot(context, parallel_streams=parallel_streams),
            ...
            compilation_monitor.set_cudagraph_capturing_enabled(previous_capturing_enabled)
                ...
                if context.cudagraph_runtime_mode == CUDAGraphMode.FULL:
                    if split_slice.start_num_tokens > 0:
                        self._update_attn_params_for_split_ubatch(
                            context, split_slice.graph_num_tokens, parallel_streams=parallel_streams
                        )
                    else:
                        self._update_attn_params_for_wrapper(context, split_slice.graph_num_tokens)
            capture_stream.synchronize()
    ...
    def _make_split_batch_metadata_inplace_parallel(
        ...
        _cached_dual_stream_metadata = getattr(self._runner, "_dual_stream_attention_metadata", None)
        _cached_dual_stream_slices = getattr(self._runner, "_dual_stream_attention_slices", None)
        _cached_dual_stream_plan = getattr(self._runner, "_dual_stream_attention_plan", None)
        _cached_dual_stream_secondary_mode = None
        if _cached_dual_stream_metadata is not None:
            _dsa_cfg = dual_stream_attention_config(getattr(self.ascend_config, "split_batch_config", None))
            _cached_dual_stream_secondary_mode = getattr(_dsa_cfg, "secondary_stream_mode", "dedicated_pair")
        ...
        for i, split_slice in enumerate(split_batch_slices):
            ...
            capture_metadata_mode = (
                "template" if split_slice.start_num_tokens > 0 and split_attention_backend == "fia" else ""
            )
            ...
            if _split_debug_enabled():
                _log_event(
                    "split_descriptor",
                    {
                        ...
                        "stream": ("parallel" if in_parallel_streams else "main"),
                        "dispatch_num_tokens": int(split_slice.graph_num_tokens),
                        "actual_num_tokens": int(split_slice.num_tokens),
                        "runtime_mode": (
                            ubatch_cudagraph_mode.name
                            if isinstance(ubatch_cudagraph_mode, CUDAGraphMode)
                            else str(ubatch_cudagraph_mode)
                        ),
            ...
            if _cached_dual_stream_metadata is not None:
                split_forward_context.dual_stream_attention_metadata = _cached_dual_stream_metadata
                split_forward_context.dual_stream_attention_slices = _cached_dual_stream_slices
                split_forward_context.dual_stream_attention_plan = _cached_dual_stream_plan
                split_forward_context.dual_stream_attention_secondary_stream_mode = _cached_dual_stream_secondary_mode
            ...
            if _split_debug_enabled():
                _log_event(
                    "inplace_parallel_execution",
                    {
                        ...
                        "metadata": _mti(split_forward_context.attn_metadata),
                        "batch_descriptor": _bd_info(split_forward_context.batch_descriptor),
                    },
                )
    ...
    def run_inplace_parallel(
        ...
        **model_kwargs,
        ...
        split_ubatch_slices = [UBatchSlice(s.request_slice, s.token_slice) for s in split_slices]
        ...
        def _run_inplace_parallel_worker(slice_idx: int) -> None:
            try:
                ...
                needs_offset = offset_capture_needed[slice_idx]
                if split_debug_active:
                    _split_debug_log_event(
                        "parallel_worker_start",
                        {
                            "slice_idx": slice_idx,
                            "path": "offset_capture" if needs_offset else "normal_replay",
                            "start_num_tokens": int(split_slice.start_num_tokens),
                            "graph_num_tokens": int(split_slice.graph_num_tokens),
                            "parallel_streams": parallel_streams,
                            **({"has_aclgraph": True} if not needs_offset else {}),
                        },
                    )
                ...
                with torch.inference_mode():
                        ...
                        with (
                            ...
                            if metadata.context.cudagraph_runtime_mode == CUDAGraphMode.FULL:
                                if split_slice.start_num_tokens > 0:
                                    ...
                                    self._update_attn_params_for_wrapper(metadata.context, split_slice.graph_num_tokens)
                    ...
                    with torch.npu.stream(target_stream):
                        results[slice_idx] = clone_split_output(trim_split_output(split_result, split_slice.num_tokens))
            except Exception as e:
                with split_error_lock:
                    split_errors.append((slice_idx, e))

        # Lazy capture temporarily rebinds process-global RoPE buffers. Run the
        # whole first-capture request serially; later requests can replay both
        # already-captured graphs concurrently without touching global state.
        offset_capture_needed = [self.needs_inplace_serial_offset_capture(metadata) for metadata in ubatch_metadata]
        sync_errors: list[Exception] = []
        if any(offset_capture_needed):
            with _INPLACE_LAZY_CAPTURE_LOCK:
                offset_capture_needed = [
                    self.needs_inplace_serial_offset_capture(metadata) for metadata in ubatch_metadata
                ]
                for slice_idx in range(num_splits):
                    _run_inplace_parallel_worker(slice_idx)
                    target_stream = self.stream_parallel if slice_idx > 0 else self.stream_main
                    try:
                        target_stream.synchronize()
                    except Exception as exc:
                        sync_errors.append(exc)
                    if split_errors or sync_errors:
                        break
        else:
        ...
        for stream in (
            self.stream_main,
            self.stream_parallel if num_splits > 1 else None,
        ):
            if stream is None:
                continue
            try:
        ...
        if split_errors:
            ...
            raise RuntimeError(
                f"inplace parallel replay worker failed at slice_idx={failed_slice_idx}: {first_error}"
            ) from first_error
        if sync_errors:
            raise RuntimeError(f"inplace parallel stream synchronization failed: {sync_errors[0]}") from sync_errors[0]
