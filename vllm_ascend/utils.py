...
def _preload_vllm_ascend_kernels() -> None:
    """Make the package-local kernel library visible to the extension loader."""
    kernel_library = os.path.join(_CUSTOM_OP_BASE_DIR, "libvllm_ascend_kernels.so")
    if not os.path.exists(kernel_library):
        return
    try:
        ctypes.CDLL(kernel_library, mode=ctypes.RTLD_GLOBAL)
    except OSError as exc:
        raise ImportError(f"Failed to load {kernel_library}: {exc}") from exc
