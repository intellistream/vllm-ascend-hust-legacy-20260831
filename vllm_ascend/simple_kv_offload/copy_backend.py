...
class NPUDmaCopyBackend:
    ...
    def _copy_loop(self) -> None:
        # Store jobs carry the compute-done event recorded by the upstream
        # worker. Waiting on it here establishes the same cross-stream
        # write-before-copy ordering as the CUDA backend. Load jobs do not
        # need that barrier because their host source is stable.
