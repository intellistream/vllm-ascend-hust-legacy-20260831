...
\bluepoint{问题场景：在 Ascend NPU 上部署大规模 MoE，expert 权重无法全量常驻 HBM}
...
\begin{itemize}
  ...
  \item \textbf{结果：} HBM 压力被缓解，但 NPU 静态复用能力下降，吞吐下降、尾延迟抖动。
\end{itemize}
\end{column}
\end{columns}
...
现有 MoE 卸载通常把 expert 权重视为动态缓存对象，通过替换、预取和流级拷贝/计算重叠降低传输开销；
但在 HBM 受限的 Ascend NPU 上，动态加载到任意 device buffer 会与\textbf{稳定权重地址、固定执行窗口、图重放和显式数据移动调度}冲突。
结果是：要么 host-to-HBM expert 加载暴露在关键路径上，要么放弃高效 NPU 执行所需的静态规整性。
\end{beamercolorbox}
\end{frame}
...
\scriptsize
\begin{itemize}
  \item \textbf{HBM 是硬约束：} expert 总权重远超单卡可用 HBM，dense 权重和 KV cache 继续挤占空间；
  \item \textbf{部署取决于驻留策略：} 没有 offload，大 MoE 只能依赖更多卡或更小模型，应用门槛显著提高；
  \item \textbf{Ascend offload 更难：} 动态加载 expert 会破坏地址稳定与图复用，必须重新设计 slot、窗口和预取流水。
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
而 expert offloading 决定了在 HBM 受限时，能否把“参数很大但激活稀疏”的 MoE 真正部署起来。
...
\vspace{0.3em}
\begin{tabularx}{\linewidth}{p{0.20\linewidth}p{0.27\linewidth}p{0.28\linewidth}X}
\toprule
\textbf{工作类型} & \textbf{现有方法怎么做} & \textbf{在我们场景下缺什么} & \textbf{导致的问题} \\
\midrule
GPU MoE offload &
把 HBM 当 expert cache；基于历史/预测做 prefetch；用 CUDA stream 和异步拷贝隐藏 H2D。 &
默认动态 kernel、动态权重对象和 stream overlap 可接受；缺少 Ascend 稳定地址、固定 shape、图重放与 MTE 编排。 &
HBM 压力转化为动态入口和搬运 stall，graph/static kernel 复用收益下降。 \\
\midrule
Ascend 全量 MoE &
vLLM Ascend 已支持 routing 到 per-expert count/grouped execution，并用 \texttt{npu\_grouped\_matmul} 执行。 &
假设 expert 权重全量在 HBM；缺少 host store、resident slot、miss/prefetch/replacement 和 phased execution。 &
后端能高效执行 grouped MoE，但无法处理“缺失 expert 如何进入 NPU”。 \\
\midrule
非 Ascend NPU MoE &
如 NPUMoE 面向 Apple ANE：把 dense/static compute offload 到 NPU，动态算子走 CPU/GPU fallback，并用 static tiers/grouped execution/graph residency。 &
目标硬件、内存体系和 runtime 不同；重点是 ANE 图执行与 fallback，不是 Ascend host-to-HBM expert offload。 &
有静态化启发，但不能直接解决 Ascend 上 expert 权重动态装载和稳定入口问题。 \\
\bottomrule
\end{tabularx}
...
\begin{beamercolorbox}[wd=\linewidth,sep=0.55em,rounded=true]{block body}
\scriptsize
\textbf{\textcolor{AccentRed}{核心转换：}}
固定 slot 提供稳定入口；deadline-aware prefetch 解决“先搬谁”；hit-first phased execution 解决“miss 已发生时怎么不整层等待”。
\end{beamercolorbox}
\end{frame}
