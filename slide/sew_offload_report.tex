...
\textbf{\textcolor{MainBlue}{GPU 对 MoE 动态性的适配}}
\footnotesize
\begin{itemize}
  \item \textbf{动态 shape 容忍度高：} GPU 运行时可通过CUDA生态可以通过动态算子、流并行和通用重排算子较好吸收 MoE routing 的动态性；
  \item \textbf{搬运机制较成熟：} expert miss 主要通过异步拷贝、prefetch、UVA 或 stream overlap 缓解传输阻塞；
  \item \textbf{优化重点：} 围绕 expert cache、预取时机和 PCIe/NVLink stall 展开。
\end{itemize}
\end{column}

\begin{column}{0.53\textwidth}
\textbf{\textcolor{MainBlue}{Ascend 上动态 MoE 带来的限制}}
\footnotesize
\begin{itemize}
  \item \textbf{静态图优势被削弱：} MoE routing 的动态性，使得Ascend无法充分利用图重放与静态kernel复用等优势，削弱了其性能优势；
  \item \textbf{固定地址优势被削弱：} offloaded expert 动态装载后，若缺少稳定 weight slot，权重地址和 buffer layout 难以复用；
  \item \textbf{计算单元协同变复杂：} expert FFN、token pack、mask、gate 和 combine 需要在 AIC/AIV 之间显式分工；
  \item \textbf{数据搬运必须显式编排：} miss expert 的加载需要与 MTE 搬运、weight slot 绑定和计算流水协同。
\end{itemize}
\end{column}
\end{columns}
