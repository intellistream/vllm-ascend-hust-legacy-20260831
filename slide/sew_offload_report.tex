...
\begin{frame}
  ...
  重点研究 Ascend 的差异化特性——包括静态图重放、固定 GM 地址空间、AIC/AIV 分离执行、显式 MTE 数据搬运以及可控的主机侧数据放置——如何重塑 MoE expert offloading 的设计空间，并据此重新设计专家权重驻留、搬运、窗口化执行与预取机制。
  \end{beamercolorbox}
\end{frame}
...
\bluepoint{背景：MoE 总参数量快速增大，单卡 HBM 越来越不够}

\vspace{0.45em}
Qwen3-30B-A3B 这类 MoE 模型每个 token 只激活少量专家，但模型总专家权重仍然需要占用大量 HBM。
当我们希望在单机单卡上服务更大 MoE，或者把更多 HBM 留给 KV cache、长上下文和并发请求时，
expert 权重就自然进入 offloading 场景。
...
\textbf{\textcolor{MainBlue}{Ascend NPU 暴露的新问题}}
\footnotesize
\begin{itemize}
  \item NPU 后端更偏好固定 shape 与稳定执行图；
  \item expert 权重地址变化会破坏 graph/static-kernel 复用；
  \item MTE/权重预取、AIC/AIV 流水是显式控制面；
  \item grouped MoE 已有，但默认假设 expert 权重常驻。
\end{itemize}
\end{column}
\end{columns}
