...
def parse_shape(value: str) -> tuple[int, int, int]:
    ...
    if len(parts) != 3:
        raise argparse.ArgumentTypeError(
            f"shape must be BxIxO, got {value!r}"
        )
    try:
        batch_size, input_dim, output_dim = (int(part) for part in parts)
    except ValueError as exc:
        raise argparse.ArgumentTypeError(
            f"shape must contain integers, got {value!r}"
        ) from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run activation_sparse_linear benchmarks across projection shapes "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Threshold mode to benchmark. Can be repeated. Defaults to scalar "
            "and row_topk."
        ),
    )
    parser.add_argument(
...
def main() -> int:
    ...
    for batch_size, input_dim, output_dim in shapes:
        ...
        for sparsity in sparsities:
            ...
            for threshold_mode in threshold_modes:
                json_output = (
                    args.output_dir /
                    f"{shape_name}_m{threshold_mode}_s{sparsity_name}_"
                    f"{args.dtype}.json"
                )
                cmd = [
                    sys.executable,
    ...
    summary = {
        ...
        "default_batch_sizes": (
            [] if args.shape else list(dict.fromkeys(default_batch_sizes))
        ),
