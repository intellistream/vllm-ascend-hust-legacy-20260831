...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Benchmark fused gate/up projection plus SiLU*up. output-dim must "
            "be 2 * intermediate_dim."
        ),
    )
...
def main() -> None:
    ...
    expected_sparse_rms = torch.sqrt(
        torch.mean(expected_sparse.to(dtype=torch.float32) ** 2)
    ).item()
    ...
    if (
        args.max_sparse_rel_err is not None
        and (
            not math.isfinite(sparse_max_rel_err)
            or sparse_max_rel_err > args.max_sparse_rel_err
        )
    ):
        failures.append(
            "packed sparse max relative error "
            f"{sparse_max_rel_err:.6g} > {args.max_sparse_rel_err:.6g}"
        )
    ...
    if direct_max_rel_err is not None and not math.isfinite(direct_max_rel_err):
        failures.append(
            f"direct sparse max relative error is not finite: {direct_max_rel_err}"
        )
    ...
    if not math.isfinite(direct_t_max_rel_err):
        failures.append(
            "direct_t sparse max relative error is not finite: "
            f"{direct_t_max_rel_err}"
        )
    if (
    ...
    if (
        ...
        and (
            not math.isfinite(direct_max_rel_err)
            or direct_max_rel_err > args.max_direct_rel_err
        )
    ):
        failures.append(
            "direct sparse max relative error "
            f"{direct_max_rel_err:.6g} > {args.max_direct_rel_err:.6g}"
        )
    if (
    ...
    if (
        args.max_direct_t_rel_err is not None
        and (
            not math.isfinite(direct_t_max_rel_err)
            or direct_t_max_rel_err > args.max_direct_t_rel_err
        )
    ):
        failures.append(
            "direct_t sparse max relative error "
            f"{direct_t_max_rel_err:.6g} > {args.max_direct_t_rel_err:.6g}"
        )
    ...
    result = {
        ...
        "env_output_tiles": {
            "VLLM_ASCEND_SPARSE_DIRECT_T_OUTPUT_TILE": os.environ.get(
                "VLLM_ASCEND_SPARSE_DIRECT_T_OUTPUT_TILE"
            ),
            "VLLM_ASCEND_SPARSE_PACKED_T_OUTPUT_TILE": os.environ.get(
                "VLLM_ASCEND_SPARSE_PACKED_T_OUTPUT_TILE"
            ),
