    ...
    *,
    ...
    if request_ids is not None:
        if len(request_ids) != count:
            raise ConversionError(f"request_ids has {len(request_ids)} rows; expected {count}")
    ...
    for index in range(count):
        request_id = request_ids[index] if request_ids is not None else f"{request_id_prefix}{index}{request_id_suffix}"
        error = errors[index]
        ...
        if not isinstance(request_itls, Sequence) or isinstance(request_itls, (str, bytes)):
        ...
        if any(isinstance(value, bool) or not isinstance(value, (int, float)) or value <= 0 for value in request_itls):
...
def main() -> None:
    ...
    if args.request_set is not None:
        ...
        if not isinstance(request_records, Sequence) or isinstance(request_records, (str, bytes)):
            raise ConversionError("request set requests must be an array")
        request_ids = [record.get("request_id") if isinstance(record, Mapping) else None for record in request_records]
    rows = convert_result(
        payload,
