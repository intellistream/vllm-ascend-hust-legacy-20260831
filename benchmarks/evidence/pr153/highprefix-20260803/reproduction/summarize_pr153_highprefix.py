#!/usr/bin/env python3
"""Validate and summarize the two-card PR 153 high-prefix crossover A/B."""
...
def transfer_summary(rows: list[dict[str, Any]], direction: str) -> dict[str, Any]:
    samples = [
        row
        for row in rows
        if row.get("event") == "transfer_done" and row.get("direction") == direction
    ]
    if not samples:
        raise ContractError(f"no completed {direction} transfers")
    latencies_ms = [
        finite_number(row.get("transfer_time_ms"), "transfer_time_ms")
        for row in samples
    ]
    byte_values = []
    for row in samples:
...
def load_run(run_dir: Path) -> dict[str, Any]:
    ...
    if config.get("vllm_dirty") is not False:
        raise ContractError(f"{run_dir} core source must be clean")
    if config.get("vllm_diff_sha256") != sha256_file(Path("/dev/null")):
        raise ContractError(f"{run_dir} core source diff must be empty")
    ...
    if f"No running processes found in NPU {physical_device_id}" not in (
        run_dir / "npu-after-shutdown.txt"
    ).read_text(encoding="utf-8"):
        raise ContractError(
            f"{run_dir} left a process on physical NPU {physical_device_id}"
        )

    metrics = {
        field: finite_number(result.get(field), f"{run_dir}/{field}")
...
def summarize_mode(runs: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for direction in ("d2h", "h2d"):
        ...
        latencies = [
            latency for run in runs for latency in run[direction]["latency_samples_ms"]
        ]
...
def build_summary(run_dirs: list[Path]) -> dict[str, Any]:
    runs = sorted(
        (load_run(path) for path in run_dirs), key=lambda run: run["lifecycle_index"]
    )
    ...
    for field in RESULT_FIELDS:
        deltas[f"{field}_percent"] = percent_delta(
            mapped[field]["mean"], native[field]["mean"]
        )
    for direction in ("d2h", "h2d"):
        for field in ("aggregate_gbps", "mean_latency_ms", "p99_latency_ms"):
            deltas[f"{direction}_{field}_percent"] = percent_delta(
                mapped[direction][field], native[direction][field]
            )
    ...
    for pair_index, pair_runs in sorted(by_pair.items()):
        ...
        for field in (
            ...
            pair[f"{field}_mapped_minus_native"] = (
                pair_modes["mapped"]["result"][field]
                - pair_modes["native"]["result"][field]
            )
        pairs.append(pair)

    by_device: dict[str, dict[str, list[dict[str, Any]]]] = defaultdict(
        lambda: defaultdict(list)
    )
    for run in runs:
        device = str(run["config"]["physical_device_id"])
        by_device[device][run["mode"]].append(run)
    device_crossovers = {}
    for device, device_modes in sorted(by_device.items()):
        if set(device_modes) != EXPECTED_MODES:
            raise ContractError(f"device {device} does not contain both modes")
        if any(len(device_modes[mode]) < 2 for mode in EXPECTED_MODES):
            raise ContractError(f"device {device} needs at least two runs per mode")
        native_device = summarize_mode(device_modes["native"])
        mapped_device = summarize_mode(device_modes["mapped"])
        device_crossovers[device] = {
            "native": native_device,
            "mapped": mapped_device,
            "mapped_vs_native": {
                f"{field}_percent": percent_delta(
                    mapped_device[field]["mean"], native_device[field]["mean"]
                )
                for field in RESULT_FIELDS
            },
        }
    ...
    for run in runs:
        item = {
            key: value for key, value in run.items() if key not in {"config", "run_dir"}
        }
        item["physical_device_id"] = run["config"]["physical_device_id"]
        item["d2h"] = strip_samples(item["d2h"])
        item["h2d"] = strip_samples(item["h2d"])
        public_runs.append(item)
    return {
        "schema_version": "pr153-highprefix-crossover-summary/v1",
        ...
        "device_crossovers": device_crossovers,
    }
...
def markdown(summary: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        "| Run | Device | Mode | Duration (s) | Req/s | Tok/s | Mean TTFT (ms) | P99 TTFT (ms) | "
    ...
    for run in summary["runs"]:
        result = run["result"]
        lines.append(
            f"| {run['lifecycle_index']} | {run['physical_device_id']} | "
            f"{run['mode']} | {result['duration']:.2f} | "
            f"{result['request_throughput']:.3f} | "
            f"{result['total_token_throughput']:.2f} | "
    ...
    lines.extend(["", "## Mode means", ""])
    lines.extend(
        [
            "| Mode | Duration (s) | Req/s | Tok/s | Mean TTFT (ms) | P99 TTFT (ms) | "
            "Mean TPOT (ms) | P99 TPOT (ms) | D2H GB/s | D2H p99 (ms) | "
            "H2D GB/s | H2D p99 (ms) | Peak NPU MB |",
            "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ]
    ...
    for mode in ("native", "mapped"):
        ...
        lines.append(
            ...
            f"{data['request_throughput']['mean']:.3f} | "
            f"{data['total_token_throughput']['mean']:.2f} | "
    ...
    lines.extend(
        [
            ...
            f"- Request throughput: {delta['request_throughput_percent']:+.2f}%",
            f"- Total token throughput: {delta['total_token_throughput_percent']:+.2f}%",
    ...
    lines.extend(["## Per-device crossover", ""])
    for device, crossover in summary["device_crossovers"].items():
        card_delta = crossover["mapped_vs_native"]
        lines.append(
            f"- NPU {device}: duration {card_delta['duration_percent']:+.2f}%, "
            f"request throughput {card_delta['request_throughput_percent']:+.2f}%, "
            f"mean TTFT {card_delta['mean_ttft_ms_percent']:+.2f}%, "
            f"p99 TTFT {card_delta['p99_ttft_ms_percent']:+.2f}%"
        )
    lines.append("")
    return "\n".join(lines)
