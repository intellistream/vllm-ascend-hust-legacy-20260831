...
def validate_plan(payload: Mapping[str, Any]) -> None:
    ...
    if stop.get("minimum_attributable_regions_for_primitive") != 2:
        raise ContractError(
            "stop_conditions.minimum_attributable_regions_for_primitive must be 2"
        )
    if stop.get("stop_when_host_device_transfer_dominates") is not True:
        raise ContractError(
            "stop_conditions.stop_when_host_device_transfer_dominates must be true"
        )
...
def _validate_declared_artifacts(run_dir: Path, payload: Mapping[str, Any]) -> None:
    ...
    for name in REQUIRED_ARTIFACTS:
        ...
        if expected != actual:
            raise ContractError(
                f"artifact hash mismatch for {name}: expected {expected}, got {actual}"
            )
...
def validate_manifest(run_dir: Path, payload: Mapping[str, Any]) -> None:
    ...
    if payload.get("evidence_label") not in EVIDENCE_LABELS:
        raise ContractError(
            f"evidence_label must be one of {sorted(EVIDENCE_LABELS)}"
        )
...
def validate_events(
    rows: Sequence[Mapping[str, Any]], *, require_recovery: bool
) -> dict[str, Any]:
    if not rows:
    ...
    for row_number, row in enumerate(rows, 1):
        request_id = require_nonempty_string(
            row.get("request_id"), f"event row {row_number}.request_id"
        )
        ...
        if event in {"transfer_start", "transfer_done"}:
            ...
            if direction not in {"d2h", "h2d"}:
                raise ContractError(
                    f"event row {row_number}.direction must be d2h or h2d"
                )
            job_id = require_nonempty_string(
                row.get("job_id"), f"event row {row_number}.job_id"
            )
            if event == "transfer_done":
                ...
                if started_at is None:
                    raise ContractError(
                        f"event row {row_number} transfer_done has no matching start"
                    )
                if timestamp < started_at:
                    raise ContractError(
                        f"event row {row_number} transfer_done precedes its start"
                    )
                ...
                if (
                    ...
                    raise ContractError(
                        f"event row {row_number}.bytes must be positive"
                    )
                ...
                if transfer_key in transfer_starts:
                    raise ContractError(
                        f"event row {row_number} duplicates an active transfer start"
                    )
    ...
    for request_id, request_rows in by_request.items():
        ...
        if selected_indices is None:
            continue
        selected_timestamps = {
            event: int(request_rows[index]["timestamp_ns"])
            for event, index in selected_indices.items()
        }
        ...
        decomposition[request_id] = {
            "copy_ms": (
                selected_timestamps["restore_done"]
                - selected_timestamps["restore_start"]
            )
            / 1_000_000,
            "restore_to_wakeup_ms": (
                selected_timestamps["scheduler_wakeup"]
                - selected_timestamps["restore_done"]
            )
            / 1_000_000,
            "wakeup_to_admission_ms": (
                selected_timestamps["admission"]
                - selected_timestamps["scheduler_wakeup"]
            )
            / 1_000_000,
            "restore_to_admission_ms": (
            ...
            / 1_000_000,
            "admission_to_first_compute_ms": (
                selected_timestamps["first_prefill_or_decode"]
                - selected_timestamps["admission"]
            )
            / 1_000_000,
            "requeue_count": sum(
                row["event"] == "requeue"
                for row in request_rows[window_start : window_end + 1]
            ),
        }
...
def validate_requests(
    ...
    for row_number, row in enumerate(rows, 1):
        request_id = require_nonempty_string(
            row.get("request_id"), f"request row {row_number}.request_id"
        )
        ...
        if status == "completed":
            ...
            raise ContractError(
                f"request row {row_number}.status must be completed or failed"
            )
    expected_count = int(require_mapping(manifest["workload"], "workload")["request_count"])
    if len(rows) != expected_count:
        raise ContractError(
            f"raw request count mismatch: expected {expected_count}, got {len(rows)}"
        )
    if completed == 0:
...
def validate_run(run_dir: Path) -> dict[str, Any]:
    ...
    event_result = validate_events(
        events,
        require_recovery=manifest["mode"]
        in {"tiering-native", "tiering-mapped"},
    )
    ...
    if unknown_event_ids:
        raise ContractError(
            "transfer events contain request IDs absent from raw requests: "
...
def validate_campaign(run_dirs: Iterable[Path]) -> dict[str, Any]:
    ...
    for mode in ("tiering-native", "tiering-mapped"):
        ...
        for field in ("ttft_ms", "tpot_ms", "latency_ms"):
            metric_values[field] = [
                _request_metric(run_dir, field) for run_dir, _ in by_mode[mode]
            ]
        metrics[mode] = {
            field: {
