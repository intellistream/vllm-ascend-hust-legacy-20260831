#!/usr/bin/env python3
"""Compare mapped-host KV gather with the main CPU-offload span-copy path.
...
steady-state samples.  Wall time is the primary metric; NPU event time is also
reported to expose stream-coverage mistakes.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("branch_development_notes/work/kv-gather-vs-span"),
    )
...
def load_runtime(args: argparse.Namespace):
    ...
    register = torch.ops._C_ascend.register_kv_cache_block_gather_host_mapping
    clear = torch.ops._C_ascend.clear_kv_cache_block_gather_host_mappings
    return torch, gather, register, clear
...
def write_results(output_dir: Path, manifest: dict[str, Any], rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Mapped-host Gather vs Main Span-copy",
        "",
