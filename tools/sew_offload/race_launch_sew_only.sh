#!/bin/bash
# Race-and-retry launcher for the SEW-only graph-capture validation (Path A).
#
...
for try in $(seq 1 "$MAX"); do
  ...
  if [ "$got_window" -eq 0 ]; then
    u=$(free_pct); u=${u:-100}
    echo "[race-sew try=$try] no free window (NPU$DEV at ${u}%, need <${FREE_THRESHOLD}%) — skipping launch, retrying" | tee -a "$LOG.race"
    continue
  fi
  echo "[race-sew try=$try] NPU$DEV at ${u}% (<${FREE_THRESHOLD}%) — launching (SEW-only, NoopOffloader)" | tee -a "$LOG.race"
  ...
  if grep -q "OUTPUT_TOKENS\|GENERATE_OK\|LOAD_OK" "$LOG"; then
    echo "[race-sew try=$try] WON — engine loaded" | tee -a "$LOG.race"; exit 0
  fi
  if grep -q "Free memory on device" "$LOG"; then
    echo "[race-sew try=$try] lost race (OOM) — retrying" | tee -a "$LOG.race"; continue
  fi
  echo "[race-sew try=$try] non-OOM exit — stopping for inspection" | tee -a "$LOG.race"; exit 2
done
