#!/usr/bin/env python3
"""Add an opt-in HCCL transport to the Stage-5 coordinator (v2).
...
This v2 keeps the original v1 patch file untouched and uses a more robust
``parse_args()`` insertion anchor.  The original Unix-RPC path remains the default.  Use
...
def transform(source: str) -> str:
    ...
    # Put the dispatch immediately after argument parsing.  The coordinator
    # has had several validation variants during Stage-5 development, so a
    # validation-specific anchor is intentionally avoided here.
    source = _replace_once(
        source,
